    ...
    public function update_product_status_pre_sync_data($last_sync_product_id = ''){
      try {
        ...
        if( $ee_prod_mapped_attrs != "" && $last_sync_product_id > 0){
          ...
          // $this->TVC_Admin_Helper->plugin_log($wpdb->last_query, 'product_sync');
          // $this->TVC_Admin_Helper->plugin_log($wpdb->last_error, 'product_sync');
          wp_reset_postdata();
        }
      } catch (Exception $e) {
        $this->TVC_Admin_Helper->plugin_log($e->getMessage(), 'product_sync');
    ...
    public function cron_todo_product_sync($request)
    {
      ...
      try {
          ...
          if (!empty($mappedCats)) {
            ...
            foreach($mappedCats as $mc_key => $mappedCat){
              ...
              for($i = 1; $i<= $total_page; $i++){
                $TVC_Admin_Helper->plugin_log("Cron - category > ".$mappedCat['name']." > total_page ".json_encode($total_page)." page > ". $i, 'product_sync');
                ...
                $TVC_Admin_Helper->plugin_log("Cron - category id ".$mc_key." gmc product name ".$mappedCat['name']." - product count - ".count($all_products), 'product_sync'); // Add logs
                if(!empty($all_products)){
        ...
        if (!empty($ee_additional_data) && isset($ee_additional_data['is_process_start']) && $ee_additional_data['is_process_start'] == true) {
          $TVC_Admin_Helper->plugin_log("Cron - product sync process start", 'product_sync');
          ...
          if (!empty($response) && isset($response['message'])) {
            $TVC_Admin_Helper->plugin_log("Cron - Batch wise auto sync process response " . $response['message'], 'product_sync');
          }
          ...
          if ($total_pending_pro == 0) {
            ...
            $ee_additional_data['product_sync_alert'] = NULL;
            $TVC_Admin_Helper->set_ee_additional_data($ee_additional_data);
            $TVC_Admin_Helper->plugin_log("Cron - product sync process done", 'product_sync');
          } else {
            /*// add scheduled cron job 
            ...
            $TVC_Admin_Helper->plugin_log("Cron - recall product sync process for remaining ". $total_pending_pro. " products", 'product_sync');
          ...
          $TVC_Admin_Helper->plugin_log("Cron - product sync process is already done", 'product_sync');
        }
        ...
        $ee_additional_data['product_sync_alert'] = $e->getMessage();
        $TVC_Admin_Helper->set_ee_additional_data($ee_additional_data);
        $TVC_Admin_Helper->plugin_log("Cron - Error - ".$e->getMessage(), 'product_sync');
      }
      return true;
    }
  }// end Class
