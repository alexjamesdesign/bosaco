...
Class TVC_Admin_Helper{
              ...
              jQuery( function() {
                jQuery( '.notice-dismiss_trigger' ).on( 'click', '.notice-dismiss', function( event, el ) {
                  ...
                  jQuery.post(tvc_ajax_url,{
                      ...
                      apiNoticDismissNonce : "<?php echo wp_create_nonce('tvc_call_notice_dismiss-nonce'); ?>",
                      dataType: "json"
                  },function( response ){                            
                  });
                });
  ...
  */  
  ...
  function get_conv_pro_link($advance_utm_medium="", $advance_linkclass="tvc-pro", $advance_linktype="anchor")
  {
    ...
    $conv_advance_plugin_link = esc_url_raw($this->get_pro_plan_site()."?utm_source=EE+Plugin+User+Interface&utm_medium=".$conv_advanced_utm_arr[$advance_utm_medium]."&utm_campaign=Upsell+at+Conversios");
    $conv_advance_plugin_link_return = "";
    $upgradetopro_text = esc_html__("Upgrade to Pro","enhanced-e-commerce-for-woocommerce-store");
