...
class Con_Settings{
  ...
  public function call_fb_conversions_api_events($args){
    try {
      if(!empty($args)){
        $header = array(
        ...
        $url = "https://graph.facebook.com/".$this->fb_api_version."/".$this->fb_pixel_id."/events";
        // Send remote request
        $args_req = array();
        $args_req["data"] = json_encode(array($args));
        $args_req["access_token"] = $this->fb_conversion_api_token;
        $args_req["partner_agent"] = "conversios";        
