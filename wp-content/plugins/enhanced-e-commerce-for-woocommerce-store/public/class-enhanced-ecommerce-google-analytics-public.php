...
class Enhanced_Ecommerce_Google_Analytics_Public extends Con_Settings{
  ...
  public function __construct($plugin_name, $version) {
     ...
     * end tvc_options
     */ 
    add_action('wp_ajax_facebook_converstion_api', array($this, 'facebook_converstion_api'));
    add_action('wp_ajax_nopriv_facebook_converstion_api', array($this, 'facebook_converstion_api'));
    add_action("wp_footer" ,array($this,'facebook_converstion_page_view'));
  }

  public function facebook_converstion_page_view(){
    if($this->fb_conversion_api_token != ""){
      // Facebook conversions API
      $gtm = new Con_GTM_Tracking($this->plugin_name, $this->version);
      $args =  array(
        "event_name" => "PageView",
        "event_time" => time(),
        "event_id"   => sanitize_text_field($this->fb_page_view_event_id),
        "event_source_url" => get_permalink(),
        "action_source" => "website",      
        "user_data" => $this->get_facebook_user_data($gtm->get_user_data())        
      );
      $this->call_fb_conversions_api_events($args);
    }
  }

  public function facebook_converstion_api(){
    if($this->fb_conversion_api_token != ""){    
      // Facebook conversions API
      $gtm = new Con_GTM_Tracking($this->plugin_name, $this->version);
      $event_name = isset($_POST['fb_event'])?sanitize_text_field($_POST['fb_event']):"";
      $cart_total = isset($_POST['cart_total'])?sanitize_text_field($_POST['cart_total']):"";
      ...
      if($event_name != ""){      
        ...
        if($cart_total >0){
          $args["custom_data"]["value"] = $cart_total;
        }
        $this->call_fb_conversions_api_events($args);
  ...
  function tvc_store_meta_data() {
    ...
    if(!empty(esc_attr($this->conv_show_badge)) && esc_attr($this->conv_show_badge) == "yes"){
      $conv_img_src =  esc_url_raw(ENHANCAD_PLUGIN_URL.'/public/images/Conversios-logo.png');
      $conv_badge_position = empty(esc_attr($this->conv_badge_position)) ? "center" : esc_attr($this->conv_badge_position);
      ...
      echo sprintf(
        '<div style="text-align: %1$s;"><a href="%2$s" target="_blank" rel="nofollow"><img style="display: inline-block" src="%3$s"/></a></div>',
        $conv_badge_position,
        $badge_img_link,
        $conv_img_src
      );
...
fbq('track', 'PageView',{},{eventID:'<?php echo esc_js($this->fb_page_view_event_id); ?>'});
</script>
...
class Con_GTM_Tracking extends Con_Settings {
    ...
    if(empty($con_ordered_item_list)){
      ...
      if($this->fb_conversion_api_token != ""){      
        $args =  array(
          ...
          "event_source_url" => get_permalink(),
          "action_source" => "website",
          "order_id" => isset($dataLayer["ecommerce"]["transaction_id"])?$dataLayer["ecommerce"]["transaction_id"]:"",
          "user_data" => $this->get_facebook_user_data($this->user_data),
          "contents" => $fb_contents,
          "custom_data" => [
            "value" => isset($dataLayer["ecommerce"]["value"])?$dataLayer["ecommerce"]["value"]:0,
            "currency" => $this->ga_LC,
    ...
    if(empty($con_view_item)){
      ...
      if($this->fb_conversion_api_token != ""){     
        $fb_contents= array(
          ...
          //"delivery_category" => isset($con_view_item["category"])?esc_js($con_view_item["category"]):""
        );     
        $args =  array(
  ...
  public function con_product_list_item_extra_tag($product, $listtype){
    ...
    if ( is_search() ) {
      ...
      $list_name = __( 'General Product List', 'enhanced-e-commerce-for-woocommerce-storer' );
    }
    $itemix = '';
